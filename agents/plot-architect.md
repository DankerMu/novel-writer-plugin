...
1. **核心冲突**：每章至少一个核心冲突
1.5. **鲜活度素材**：每章契约必须填写「互动设计」「质感锚点」「声音节拍」三个 section——这是给 ChapterWriter 的行文素材，不是约束检查项。独角戏和多人戏都可以，关键是每个场景的动态要具体（独角戏写主角跟什么处境在博弈），物件要有个人历史附着，声音方向要明确。世界观信息优先通过角色动机驱动流出（角色在搜索/争论/盘算），禁止旁白单方面灌入超过 200 字的设定段
...
从叙述性大纲自动派生每章的**质感导向**契约。核心原则：**鲜活感来自具体性和态度感。** 每一句描写都要有具体细节（不是"一扇门"而是"贴着小广告的防盗大门"），每一段叙述都要带叙述者的态度（不是中立复述而是有观点的讲述）。契约的职责是给 ChapterWriter 提供足够具体的素材（人物动态、物件细节、声音方向），让它写出有生活痕迹的文字而非"能力达标的平庸文字"。
...
- `涉及角色`：从「事件中自然流露的角色特质」+「互动设计」的参与者列表合并提取
...
- `interaction_scenes`：从「互动设计」提取场景列表（参与者+动态）
- `texture_seeds`：从「质感锚点」提取物件列表
- `voice_beats`：从「声音节拍」提取节拍列表
