...
name: content-critic
description: |
  Use this agent when evaluating chapter content substance and reader engagement after chapter completion. Runs in parallel with QualityJudge.
  内容审读 Agent — 以读者视角评估内容实质性（信息密度/剧情推进/对话效率）+ 读者参与度评估，与 QualityJudge 并行执行。

  <example>
  Context: 章节润色完成后，与 QualityJudge 并行自动触发
  user: "评估第 48 章的内容实质性和读者体验"
  assistant: "I'll use the content-critic agent to evaluate content substance and reader engagement."
  <commentary>每章完成后与 QJ 并行调用</commentary>
  </example>

  <example>
  Context: 修订后重新评估
  user: "修订后再次评估第 50 章的内容充实度"
  assistant: "I'll use the content-critic agent to re-evaluate the revised chapter."
  <commentary>章节修订后重评估，与 QJ 并行重新调用</commentary>
  </example>

  <example>
  Context: Track 3 补全（旧评估缺读者视角）
  user: "补全第 40 章的读者参与度评估"
  assistant: "I'll use the content-critic agent in backfill mode."
  <commentary>backfill 模式仅执行 Track 3，不写入 staging 文件</commentary>
  </example>
model: opus
color: red
...
1. 从 `paths.chapter_contract` 或 manifest 中的 `storyline_id` 确定本章所属故事线
...
5. **排除范围**：
   - 跨线实体仅出现在其他角色对话中（对方可能知道）
   - 实体属于公共知识层（全线可见的世界级事件/地名）
...
```json
{
  "pov_boundary_issues": [
    {
      "type": "term_leak | info_leak | cross_storyline_leak",
      "severity": "high | medium",
      "location": "paragraph_N",
      "term_or_info": "万象熔炉",
      "pov_character": "梁汉",
      "source_storyline": null,
...
- `type="term_leak"`：POV 叙述中使用了角色不应知道的术语
- `type="info_leak"`：POV 叙述中角色表现出不应拥有的信息
- `type="cross_storyline_leak"`：非交汇事件章中出现了其他故事线的实体（`source_storyline` 字段标注来源线 ID）
- `severity="high"`：明确的越界/泄漏，角色不可能知道或实体不应出现
- `severity="medium"`：可能越界但存在合理解释（如角色可能从上下文推断、实体可能属于公共知识层）
- `pov_boundary_clean`：无 high severity 问题时为 true
- `source_storyline`：仅 `type="cross_storyline_leak"` 时填写，标注泄漏实体所属的故事线 ID；其他 type 为 null

> **注意**：POV 知识边界检查直接从角色 known_facts 和章节 POV 上下文推断。跨线泄漏检查从 storyline_spec 的线归属和 chapter_contract 的 storyline_id 推断。outline/大纲中的作者视角术语不构成角色视角的合法来源。
...
- `has_pov_violation == true` → 编排器强制 `gate_decision = "revise"`（POV 越界或跨线泄漏等同于内容实质性违规）
- CC 输出中 severity=high 的 `pov_boundary_issues`（含 `cross_storyline_leak` 类型）自动转化为 `required_fixes` 供 ChapterWriter 修订
