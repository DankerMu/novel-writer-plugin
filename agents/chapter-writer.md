...
- **术语一致性检查**：若 `world/terminology.json` 存在，运行 `scripts/lint-terminology.sh` 扫描正文。warning 级别命中逐条检查：确认为漂移（非合法变体/别名）则统一为 canonical 形式；角色对话中的刻意别称豁免
...
5. **引号格式复检**：确认前置清洗后无遗漏——全文不应存在任何非中文双引号字符
...
6.11. **AI 句式原型扫描替换**：逐段扫描 5 类 AI 句式原型（作者代理理解/模板化转折/抽象判断/书面腔入侵/否定-肯定伪深度），识别后按 `ai-blacklist.json` 中 `ai_sentence_pattern` 的 `replacement_strategy` 定向替换。"不是X，而是Y"/"不是X，是Y"句式同时命中 template_transition 和 negation_affirmation 双重检测。第一人称"我知道他在…"豁免
...
9. **通读确认**：通读全文确认语义未变、角色语癖和口头禅未被修改

### Phase 2 约束

**优先级分层**：
- **P0（必做）**：模型 artifact 清除 + 引号格式统一 + 格式规则检查（前置清洗）、黑名单替换（6.3）、叙述连接词清除（6.5）、标点频率修正（6.4，含破折号绝对零容忍）、AI 句式原型替换（6.11）
- **P1（优先）**：修饰词去重（6.6）、四字词组密度（6.7）、句式分布（6.6）、比喻密度检查（6.12）
- **P2（条件触发）**：形容词密度（6.8）、感叹号频率（6.9）、抽象→具体（6.10）— 仅在 P0+P1 修改量可控时执行
...
- **二次润色风格保持**：polish_only 模式下注意控制修改范围，避免过度润色导致风格漂移
