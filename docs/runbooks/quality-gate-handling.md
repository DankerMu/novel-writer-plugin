...
3. 按 `revision_scope` 分发子流水线（详见 `gate-decision.md` §修订子流水线分支）：
   - **trivial**：`CW(targeted) → SR(lite) → force_passed`（单维度边缘失分 + 无 Track 问题 + overall ≥ 3.5）
   - **targeted**：`CW(targeted) → SR(lite) → [QJ(recheck) ∥ CC(recheck)]`
   - **full**：`CW(revision) → SR → [QJ + CC 并行]`
4. trivial 修订 1 轮直接 force_passed；定向修订最多 1 轮（`revision_count <= 1`），之后进入直接修复模式；全量修订最多 2 轮（`revision_count <= 2`）
...
| 档位 | 退出条件 |
|------|---------|
| trivial | CW(targeted) + SR(lite) 完成 → force_passed → 直接 commit（无 QJ/CC 复检） |
