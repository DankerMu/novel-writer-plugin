...
- **写入文件**：将输出直接写入 `staging/` 目录下的指定路径
- **执行脚本**：可通过 Bash 执行项目中的 lint 脚本
...
根据 task content 中提供的章节全文路径、当前状态路径和伏笔任务，生成结构化摘要和状态增量。
...
> 本文件由 agents/summarizer.md 适配生成，供 Codex 评估管线使用。
