#!/usr/bin/env bash
# PreToolUse hook: sliding window checkpoint enforcement (two-phase).
#
# Phase 1 — TRIGGER: When a chapter commit (mv/cp from staging/) hits a
#   checkpoint (≥10 and %5==0), create a marker file and inject the
#   systemMessage. The commit is ALLOWED.
#
# Phase 2 — GATE: On the next chapter commit, if the marker exists but
#   logs/continuity/latest.json has NOT been updated since the trigger,
#   DENY the commit — forcing the orchestrator to complete the check first.
#   Once the report is written, the marker is cleared and the commit proceeds.
#
# Non-chapter Bash commands are always allowed (no interference with
# checkpoint writes, staging cleanup, etc.).

set -euo pipefail

command -v jq >/dev/null 2>&1 || exit 0

input="$(cat)"

hook_event="$(echo "$input" | jq -r '.hook_event_name // ""')"
tool_name="$(echo "$input" | jq -r '.tool_name // ""')"

[ "$hook_event" = "PreToolUse" ] || exit 0
[ "$tool_name" = "Bash" ] || exit 0

cmd="$(echo "$input" | jq -r '.tool_input.command // ""')"
cwd="$(echo "$input" | jq -r '.cwd // ""')"
project_dir="${cwd:-$(pwd)}"

# Only act inside a novel project
[ -f "$project_dir/.checkpoint.json" ] || exit 0

# --- Extract chapter number from commit command ---
# Match: mv/cp staging/chapters/chapter-NNN.md → chapters/
if ! echo "$cmd" | grep -qE '(mv|cp)\b.*staging/chapters/chapter-[0-9]{3}\.md'; then
  exit 0
fi
chapter_num="$(echo "$cmd" | grep -oE '[^/]chapters/chapter-([0-9]{3})\.md' | head -1 | grep -oE '[0-9]{3}')" || true
[ -n "$chapter_num" ] || exit 0
chapter_num=$((10#$chapter_num))

marker="$project_dir/logs/.sliding-window-pending"
report="$project_dir/logs/continuity/latest.json"

# === Phase 2: GATE — block if prior check is still pending ===
if [ -f "$marker" ]; then
  if [ -f "$report" ] && [ "$report" -nt "$marker" ]; then
    # Report updated after trigger — check complete, clear marker
    rm -f "$marker"
  else
    # DENY: check not done yet
    pending_ch="$(cat "$marker" 2>/dev/null || echo "?")"
    jq -n \
      --arg msg "⛔ 滑窗校验未完成。第 ${pending_ch} 章触发的滑窗一致性校验尚未执行（logs/continuity/latest.json 未更新）。请先完成 SKILL.md Step 8 的校验和修复流程，再提交下一章。" \
      --arg reason "sliding window check pending since chapter ${pending_ch}" \
      '{systemMessage: $msg, hookSpecificOutput: {hookEventName: "PreToolUse", permissionDecision: "deny", permissionDecisionReason: $reason}}'
    exit 0
  fi
fi

# === Phase 1: TRIGGER — create marker if checkpoint ===
[ "$chapter_num" -ge 10 ] || exit 0
[ $(( chapter_num % 5 )) -eq 0 ] || exit 0

ws=$(( chapter_num - 9 ))
[ "$ws" -ge 1 ] || ws=1

# Create marker (content = triggering chapter number)
mkdir -p "$project_dir/logs"
echo "$chapter_num" > "$marker"

# Build chapter file list
files=""
for i in $(seq "$ws" "$chapter_num"); do
  files="${files}chapters/chapter-$(printf '%03d' "$i").md, "
done
files="${files%, }"

msg="⚠️ 【滑窗校验点】第 ${chapter_num} 章已提交，触发滑窗一致性校验（窗口 ch${ws}–ch${chapter_num}）。

完成本章 commit 流程后，必须在写下一章之前执行以下操作（详见 SKILL.md Step 8）：

1. 读取窗口内所有章节原文（${files}）+ 对应大纲区块（outline.md 中各章 ### 段落）+ 对应章节契约（chapter-contracts/*.md）
2. 正文↔契约/大纲对齐检查（逐章）：「事件」是否完整呈现、「冲突与抉择」是否落地、「局势变化」章末状态是否一致、「验收标准」各条是否满足、大纲 Storyline/POV/Location 是否匹配、大纲 Foreshadowing 伏笔动作是否体现
3. 跨章连续性检查：角色位置/状态连续性、时间线矛盾、世界规则合规性、伏笔推进一致性、跨线信息泄漏
4. 可选辅助：NER 实体抽取（scripts/run-ner.sh，脚本优先，LLM fallback）
5. 报告落盘：logs/continuity/continuity-report-vol-*-ch${ws}-ch${chapter_num}.json + 覆盖 logs/continuity/latest.json
6. 自动修复：对可修复问题（事实性矛盾、连续性断裂、正文偏离契约/大纲）直接编辑受影响章节原文；不可自动修复的问题（剧情逻辑矛盾等）列出并提示用户
7. 完成后继续写下一章（hook 会检测 latest.json 更新，自动放行后续 commit）"

jq -n --arg msg "$msg" \
  '{systemMessage: $msg, hookSpecificOutput: {hookEventName: "PreToolUse", permissionDecision: "allow"}}'
