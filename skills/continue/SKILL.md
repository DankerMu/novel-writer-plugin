...
for chapter_num in range(start, start + remaining_N):
  ...
  3. QualityJudge Agent → 质量评估（Track 1+2+3 统一评估）
     ...
     输入: quality_judge_manifest（inline 计算值 + 文件路径；cross_references 来自 staging/state/chapter-{C:03d}-crossref.json；track3_mode 由 context-assembly 2.6 判定）
     输出: staging/evaluations/chapter-{C:03d}-eval-raw.json（QJ 直接落盘；含 overall_raw + overall_weighted（有 platform_guide 且含评估权重时）+ overall（= overall_weighted 或 overall_raw）+ platform_weights + reader_evaluation（Track 3 读者评估，lite 模式精简输出，可为 null））
