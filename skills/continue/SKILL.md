...
| `drafting` | ChapterWriter 执行中 | 检查 staging 文件决定从 CW 或 SR 恢复 |
...
for chapter_num in range(start, start + remaining_N):
  ...
  1. ChapterWriter Agent → 生成初稿（Phase 1 纯创作 + 语域微注入，无合规润色）
     输入: chapter_writer_manifest（inline 计算值 + 文件路径；不含 ai_blacklist / style_guide / ai_blacklist_top10）
