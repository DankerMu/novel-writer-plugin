...
- eval_backend: 评估后端（"codex" | "opus"；缺失视为 "opus"）
```
...
for chapter_num in range(start, start + remaining_N):
  ...
  2. Summarizer → 生成摘要 + 权威状态增量 + 串线检测
     **按 eval_backend 分支**:
     - **eval_backend = "opus"**（默认）:
       Task(subagent_type="summarizer", model="opus") — 现有行为不变
     - **eval_backend = "codex"**:
       2a. 组装 task content:
           `Bash("python3 ${CLAUDE_PLUGIN_ROOT}/scripts/codex-eval.py staging/manifests/chapter-{C:03d}-manifest.json --agent summarizer --project <root>")`
       2b. Codex 执行:
  ...
  3. QualityJudge + ContentCritic **并行**评估
     ...
     **eval_backend = "opus"**（默认）:
     3a. QualityJudge Agent → Track 1 合规 + Track 2 评分
         Task(subagent_type="quality-judge", model="opus")
     ...
     3b. ContentCritic Agent → Track 3 读者参与度 + Track 4 内容实质性
         Task(subagent_type="content-critic", model="opus")
     ...
     **eval_backend = "codex"**:
     3a-codex. 组装 QJ + CC task content（并行）:
         `Bash("python3 ${CLAUDE_PLUGIN_ROOT}/scripts/codex-eval.py staging/manifests/chapter-{C:03d}-manifest.json --agent quality-judge --project <root>")`  ─┐ 并行
         `Bash("python3 ${CLAUDE_PLUGIN_ROOT}/scripts/codex-eval.py staging/manifests/chapter-{C:03d}-manifest.json --agent content-critic --project <root>")` ─┘ 并行
