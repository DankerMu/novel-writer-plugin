...
| `refined` | StyleRefiner 完成 | 普通/全量修订：从 Summarizer 恢复；定向修订（`revision_scope=="targeted"`）：从 [Sum ∥ QJ ∥ CC] 三路并行恢复（检查输出存在性，仅重跑缺失） |
| `drafted` | Summarizer 已完成 | 从 QualityJudge + ContentCritic 并行恢复（定向修订跳过此阶段） |
...
for chapter_num in range(start, start + remaining_N):
  ...
  5. 质量门控决策（Gate Decision Engine）:
     门控决策合并 QJ + CC 结果（详见 `references/gate-decision.md`）：
       ...
       修订上限兜底：定向修订 1 轮后未通过 → 直接修复模式（Task agent + SR + Sum，跳过 QJ/CC）+ force_passed；全量修订 2 轮后 overall ≥ 3.0 且无 high violation 且无硬门 fail 且无 substance_violation 且无黄金三章 engagement < 3.0 → force_passed

     **修订子流水线分支**（gate_decision="revise" 且可进入修订：定向 `revision_count < 1`，全量 `revision_count < 2`）：
     ...
     5a. **revision_scope = "targeted"**（定向修订，约 35-45K tokens）：
         ...
         **SR 完成后、三路并行前的 manifest 预处理**（同步执行，不涉及 LLM 调用）：
         - Sum diff 行数占比检查（读取 revision-diff JSON 计算 `修改行数 / 总行数`）：
           - diff ≥ 30% → 降级为 Summarizer 全量模式（不传 patch_mode）
           - diff < 30% → Sum manifest 追加: `patch_mode=true` + `paths.previous_summary` + `paths.previous_delta` + `paths.revision_diff`
         ...
         **三路完成后**：
         - 验证三份输出均存在且可解析为合法 JSON（任一缺失/解析失败 → 按 Step 1.6 重试对应 agent）
         - **Escalation 处理**：若 QJ 或 CC 任一输出含 `recheck_escalated: true` → 丢弃 QJ+CC 输出（eval-raw + content-eval-raw），降级为 revision_scope="full"，从全量 [QJ + CC 并行] 重跑（不重跑 CW/SR/Sum，仅重跑评估；Sum 输出保留）
         - 进入 Step 5 门控决策合并
