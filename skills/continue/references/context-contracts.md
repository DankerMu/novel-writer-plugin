...
## AudienceEval manifest

```
audience_eval_manifest = {
  # ── inline（编排器计算） ──
  chapter: int,
  volume: int,
  platform: str | null,           # 从 style-profile.json 的 platform 字段提取（fanqie | qidian | jinjiang | null）
  excitement_type: [str] | null,  # 从 chapter_contract 提取（可选）
  is_golden_chapter: bool,        # chapter <= 3

  # ── paths（subagent 自读） ──
  paths: {
    chapter_draft: "staging/chapters/chapter-{C:03d}.md",                      # 评估对象（staging 中的润色后版本）
    recent_summaries: ["summaries/chapter-{C-1:03d}-summary.md", ...],         # 近 2 章（建立阅读连续感；首章为空数组）
    style_profile: "style-profile.json",                                       # 确认 platform + genre
    chapter_contract: "volumes/vol-{V:02d}/chapter-contracts/chapter-{C:03d}.json",  # 可选（提取 excitement_type）
  }
}
```

> **精简输入设计**：AudienceEval 不需要 world_rules、character_contracts、storyline_spec 等——读者不会带着设定集来看书。输入仅包含读者实际能感知到的内容：正文 + 前情提要 + 平台上下文。

---

## AudienceEval 返回值契约

AudienceEval 为只读 Agent，不写文件。其返回的结构化 JSON 由编排器消费并写入 `staging/evaluations/chapter-{C:03d}-audience.json`。

关键返回字段：
- `persona` — 使用的读者画像标识
- `reader_scores` — 6 维度读者评分（continue_reading / hook_effectiveness / skip_urge / confusion / empathy / freshness，各含 score/reason/evidence）
- `overall_engagement` — 加权均值（权重因平台而异）
- `suspicious_skim_paragraphs` — 1-3 个最可能被跳读的段落（paragraph_index / opening_words / reason / severity）
- `emotional_arc` — 情绪采样点 + 弧线形状 + 最低/最高点位置
- `platform_signal` — 平台特定预测信号 + one_line_verdict
- `golden_chapter_flags` — 黄金三章专属警告（仅 is_golden_chapter=true 时输出）
- `reader_feedback` — 第一人称 2-3 句读后感

**门控参与**：编排器在门控决策中叠加 AudienceEval 结果（详见 `gate-decision.md` § AudienceEval 叠加门控）：
- 黄金三章：overall_engagement < 3.0 → 至少 revise
- 普通章：QJ pass 但 overall_engagement < 2.5 → 降为 polish
- AudienceEval 失败/超时 → 仅用 QJ 门控（优雅降级）
